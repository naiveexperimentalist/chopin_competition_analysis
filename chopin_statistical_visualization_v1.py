...
class ChopinStatisticalVisualization:
    ...
    def visualize_clustering_results(self, stage: str = 'final', save_path: str = None):
        ...
        try:
            clusters_df, cluster_stats, cluster_centers, scaler = \
                self.clustering_analyzer.kmeans_clustering_participants(stage, n_clusters=5)
        ...
        # 4. Tabela najlepszych w każdym klastrze
        ax4 = axes[1, 1]
        ax4.axis('off')
        ...
        for cluster_id in sorted(clusters_df['cluster'].unique()):
            cluster_data = clusters_df[clusters_df['cluster'] == cluster_id]
            top_in_cluster = cluster_data.nlargest(1, 'mean_score')
            
            if not top_in_cluster.empty:
                row = top_in_cluster.iloc[0]
                participant = row['participant'].split(':')[1].strip() if ':' in row['participant'] else row['participant']
                table_data.append([
        ...
        table = ax4.table(cellText=table_data,
                        colLabels=['Klaster', 'Najlepszy uczestnik', 'Ocena', 'Rozmiar'],
                        cellLoc='left',
                        loc='center',
                        colWidths=[0.2, 0.45, 0.15, 0.15])
        table.auto_set_font_size(False)
        table.set_fontsize(10)
        table.scale(1, 2)
        
        ax4.set_title('Najlepsi uczestnicy w każdym klastrze',
                     fontsize=14, fontweight='bold', pad=20)
        
        plt.suptitle(f'Analiza clusteringu uczestników - {stage}',
                    fontsize=16, fontweight='bold', y=1.00)
        plt.tight_layout()
    ...
    def create_comprehensive_statistical_report(self, output_dir: str = 'statistical_visualizations'):
        """
        ...
        if self.controversy_analyzer:
            ...
            self.visualize_controversy_heatmap('final', f'{output_dir}/score_diversity_heatmap.png')
            
            print("  - Analiza zróżnicowania ocen")
            self.visualize_controversy_analysis(f'{output_dir}/score_diversity_analysis.png')
            
            print("  - Analiza outlierów")
            self.visualize_outliers_analysis(f'{output_dir}/outliers_analysis.png')
        
        # Stabilność rankingu
        if self.statistical_analyzer:
            print("  - Stabilność rankingu")
            self.visualize_ranking_stability('final', f'{output_dir}/ranking_stability.png')
            
            print("  - Istotność statystyczna")
            self.visualize_statistical_significance('final', f'{output_dir}/statistical_significance.png')
            
            print("  - Pair-wise agreement")
            self.visualize_pairwise_agreement(f'{output_dir}/pairwise_agreement.png')
        
        # Clustering
        if self.clustering_analyzer:
            print("  - Wyniki clusteringu")
            self.visualize_clustering_results('final', f'{output_dir}/clustering_results.png')
            
            print("  - PCA sędziów")
            self.visualize_pca_judges(f'{output_dir}/pca_judges.png')
